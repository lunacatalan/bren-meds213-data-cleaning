...
snowsurvey_fixed <- snowsurvey_csv %>%
  mutate(Snow_cover = ifelse(Snow_cover == ".",
                NA,
                Snow_cover))
```
...
snowsurvey_fixed <- snowsurvey_fixed %>%
  mutate(Snow_cover = ifelse(Snow_cover == "n/a",
                NA,
                Snow_cover))
...
snowsurvey_fixed <- snowsurvey_fixed %>%  
  mutate(Snow_cover = ifelse(Snow_cover == "unk",
                NA,
                Snow_cover))
...
snowsurvey_fixed <- snowsurvey_fixed %>%
  mutate(Snow_cover = ifelse(Snow_cover == "<1",
                0,
                Snow_cover))
...
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snow_cover = ifelse(Snow_cover > 100,
         NA,
         Snow_cover))
...
species_long <- species_csv %>% 
  pivot_longer(
    ...
    values_transform = list(species_count = as.character) # transform the column species_count
  )
...
species_presence <- species_long %>% 
  mutate(presence = ifelse(species_count == 0, 
                           0, 
                           1))
